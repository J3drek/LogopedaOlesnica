...
//TABBED
clickersList.forEach(clicker => {clicker.addEventListener('click', function(event){
    event.preventDefault();
    clickersList.forEach(clicker => clicker.classList.remove('active--clicker'));
    changersList.forEach(changer => {
        changer.classList.remove('active--changer');
        if(Number(changer.dataset.number) === [...clickersList].indexOf(event.target) + 1){
...
contact.addEventListener('click', function(event) {
    document.getElementById('myModal').classList.remove('is-hidden','is-visuallyHidden');
    main.classList.add('is-blurred');
});
closeModal.addEventListener('click', function(event) {
    event.preventDefault();
    document.querySelector('.Modal').classList.add('is-hidden','is-visuallyHidden');
